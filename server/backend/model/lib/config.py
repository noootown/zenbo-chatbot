...
def params_setup(cmdline=None, model_num=0):
    ...
    parser.add_argument('--model_2_name', type=str, default='movie_subtitles_en', help='model name, affects data, model, result save path')
    parser.add_argument('--scope_name', type=str, help='separate namespace, for multi-models working together')
    parser.add_argument('--scope_name1', type=str, help='separate namespace, for multi-models working together')
    ...
    parser.add_argument('--batch_size', type=int, default=batch_size, help='Batch size to use during training.')
    parser.add_argument('--vocab_size', type=int, default=vocab_size, help='Dialog vocabulary size.')
    parser.add_argument('--size', type=int, default=model_size, help='Size of each model layer.')
    ...
    # predicting params
    parser.add_argument('--beam_size', type=int, default=beam_size, help='beam search size')
    parser.add_argument('--antilm', type=float, default=antilm, help='anti-language model weight')   # 0.6
    parser.add_argument('--n_bonus', type=float, default=n_bonus, help='bonus with sentence length')   # why ori-type is int

    # environment params
    parser.add_argument('--gpu_usage', type=float, default=gpu_usage, help='tensorflow gpu memory fraction used')
    ...
    if model_num:
        args.model_name = args.model_2_name
        if not args.scope_name1: args.scope_name = args.model_2_name
